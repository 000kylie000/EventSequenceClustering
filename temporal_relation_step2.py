                    ...
                    for k,v in relation_used.items():
                        if len(v) > 0:
                            for j in range(len(v)):
                                ...
                                self.relation[k][v[j]] -= grad_relation * 0.003 # 0.25，0.3， 0.35
...
for iter in range(iter_nums):
    ...
    for i in range(logic_model.num_formula-1):
        for k,v in logic_model.relation[str(i)].items():
            valid.append(int(torch.max(v,dim=0)[1].numpy()))
    print(valid)
    record.append(valid)
    print(record)
    ...
    if len(record) >= 6:
        for i in range(-6,-1,1):
            equal = True
            if record[-1]!=record[i]:
                equal = False
            if equal:
                count += 1
        print(count)
        if len(record_single) == len(record[-1]):
            break
        if count == 5:
            break
